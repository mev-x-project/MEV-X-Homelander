...
contract HomelanderUniV4Plugin is BaseHook, Ownable {
	...
	function _afterSwap(
		...
		if (isArbPossible) {
			try mevxExecutor.executeRoute(encodedRoute, pools, amountIn, profitToken, address(profitDistributor)) {
				try profitDistributor.distributeProfit(configId, profitToken, sender) {} catch {}
			} catch {}
		}
